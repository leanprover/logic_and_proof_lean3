...
- /antisymmetry/: if $a \leq b$ and $b \leq a$ then $a = b$, for every
  $a$ and $b$ in $A$.
...
5. Let the relation $\sim$ on the natural numbers $\NN$ be defined as follows:
   if $n$ is even, then $n \sim n+1$, and if $n$ is odd, then $n \sim n-1$. Furthermore,
   for every $n$, $n \sim n$. Show that $\sim$ is an equivalence relation. What
