...
of the assumption or theorem that is used. If the justification is an
identity that is used at the "top level", you can just give the name
of the identity. To apply an identity =foo= within an expression, use
=by rewrite foo=, or =by rewrite -foo= to apply it in the reverse
direction.  As usual, the syntax is finicky; notice that there are no
