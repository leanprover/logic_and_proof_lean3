...
2.  $\ex {x, y} (\fn{adj}(x, y) \wedge \fn{green}(x) \wedge
    \fn{green}(y))$
3.  $\ex x ((\ex z \fn{right{\dash}of}(z, x)) \wedge (\fa y
...
/reductio ad absurdum/ rule we could prove $A$ from
...
1. In a first-order language with a binary relation, $R(x,y)$,
   ...
   - $\ex x \fa y R(x, y)$
   - $\ex y \fa x R(x, y)$
   - $\fa {x,y} (R(x,y) \wedge x \neq y \to \ex z (R(x,z) \wedge
     R(z,y) \wedge x \neq z \wedge y \neq z))$
...
2. Create a 4 x 4 "dots" world that makes all of the following
   ...
   - $\ex {x, y} (\fn{adj}(x, y) \wedge \fn{green}(x) \wedge \fn{green}(y))$
