...
statement of the definition, and that $\equiv$ is written as an infix
symbol. Notice also that even though the relation is written with the
symbol $\equiv$, it is the only symbol occurring in the definition;
mathematical practice favors natural language to describe its
properties.
...
statement in Lean. We will also leave you with an exercise: by a
...
equality. You should check that the following mathematical informal
...
- the relation on lines in the same plane, given by "$x$ and $y$ are
  parallel"
...
- /asymmetry/: if $a \leq b$ and $b \leq a$ then $a = b$.
-----
...
order/ (also called a /linear order/) if it has the following
additional property:
...
not a partial order, since it is not reflexive. It is, rather, and
...
/antisymmetric/: for every $a$ and $b$, $a < b$ implies $b \nless a$.
...
< b$ and $b < c$, violating antisymmetry. So $a \neq c$, as required.
...
and $x_2 \leq x_1$. By asymmetry, $x_1 = x_2$.
-----
...
minimal element is minimum, if false. As an example, consider the
...
meant is that each line follows from the preceeding
one. There is a way of expressing this, with the phrase "it
suffices to show." The following presentation comes closer to
...
** Carrying out calculations formally.
...
the following identities for any real numbers $x$, $y$, and $z$:
- commutativity of addition: $x + y = y + x$
- ...
Note that $x$, $y$, and $z$ can, in particular, be integers or
rational numbers as well.

(Do a natural deduction derivation.)

(Do an example in Lean with calc.)

Let us consider a more interesting example. 

(Motivate the sums of squares theorem with some examples.)
...
(Do it in Lean with a calc proof.)

(Introduce rewrite, briefly.)

(Combine calc and rewrite.)

** Calculations with Propositions and Sets

Boolean algebras. Identities. 
