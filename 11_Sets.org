...
/formal/ proofs we've written when writing natural deduction proof
trees. In informal proofs we focus on readability. We use English
words instead of mathematical formulas to structure the proof, and we
write paragraphs instead of trees.
...
Returning to our examples of families of sets, we can compute the
union and intersection of the families in these examples. For the
first example, $\bigcup_{n \in \NN} A_n$ is the set of all living
people, and $\bigcap_{n \in \NN} A_n = \emptyset$. Also, $\bigcup_{r
\in \RR} B_r = \RR_{>0}$, the set of all positive real numbers, and
$\bigcap_{r \in \RR} B_r = \emptyset$. For the last example, we have
...
Before we can introduce the next operation on sets, we need to talk
about pairs. A pair, or /ordered pair/, of two object $a$ and $b$ is
denoted $(a, b)$. Here $a$ is the /first component/ and $b$ is the
/second component/ of $(a, b)$. Two pairs are only equal if the first
component are equal and the second components are equal. So $(a, b) =
(c, d)$ if and only if $a = c$ and $b = d$.

We can take pairs as primitive notions, which is usually done in type
theory. This is also the method used in Lean. However, in set theory
we can define pairs as a particular set, namely \( (a, b) = \{\{a\},
\{a, b\}\} \). Note that in this definition, an ordered pair $(a, b)$
doesn't always have two elements, namely if $a = b$, then
\begin{equation*}
(a, a) = \{\{a\},\{a, a\}\} = \{\{a\},\{a\}\} = \{\{a\}\}.
\end{equation*}
#+HTML: <hr>
...
*Theorem*. Using the definition \( (a, b) = \{\{a\}, \{a, b\}\} \) we have
...
that $a$ and $b$ are both equal to $c$, contradiction $a \neq
b$. Since $\{c\}\in L$, $\{c\}$ must be equal to $\{a\}$, which means that $a =
c$. We know that $\{a, b\} \in R$, and since we know $\{a, b\}\neq
\{c\}$, we conclude $\{a, b\} = \{c, d\}$. This means that $b \in\{c,
...
Using ordered pairs we can also define /ordered triples/ $(a, b, c)$ as
$(a, (b, c))$. Then we can prove that $(a, b, c) = (d, e, f)$ if and
only if $a = d$, $b = e$ and $c = f$, which you are asked to do in
the exercises. We can also define ordered $n$-tuples which is a
sequence of $n$ objects in a similar way.

Given two sets $A$ and $B$, we can define the /cartesian product/ $A
\times B$ of these two types as the set of all pairs where the first
...
Given a set $A$ we can define the /power set/ $\mathcal P(A)$
to be the set of all subsets of $A$. In set-builder notation we can
write this as
\begin{equation*}
\mathcal P(A) = \{B \mid B \subseteq A\}.
\end{equation*}
Similar to the cartesian product, if $A$ is a subset of $\mathcal U$,
we don't necessarily have that $\mathcal P(A)$ is a subset of the
same domain. However, it is a subset of $\mathcal P(\mathcal U)$.
