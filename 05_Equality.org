...
- $\overline{A} \cap (A \cup B) = A$
- $\overline{A} \cup (A \cap B) = A$
...
example : (A ∩ -B) ∪ B = A ∪ B :=
calc
  (A ∩ -B) ∪ B = (A ∪ B) ∩ (-B ∪ B) : union.distrib_right
           ... = (A ∪ B) ∩ univ     : comp_union_self
           ... = A ∪ B              : inter_univ
#+END_SRC
...
- $\neg{A \wedge B} = \neg A \vee \neg B$
- $\neg{A \vee B} = \neg A \wedge \neg B$
- $\neg{A} \wedge (A \vee B) = A$
- $\neg{A} \vee (A \wedge B) = A$
