...
2. Using the mnemonoic =F= (Father), =M= (Mother), =D= (Daughter), =S=
   ...
   youngest, and only the mother or father can be the oldest. Part (d)
   is tricky; try to write down a formula that describes all the
   possibilities that are not ruled out by the information.
